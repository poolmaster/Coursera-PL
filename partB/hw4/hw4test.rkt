...
(define a 2)
(define ll (list (cons 1 "a") (cons 2 "b") (cons 3 "c") (cons 4 "d")))
(define (fassoc v) ((cached-assoc ll 3) v))
...
(define tests
  (test-suite
   ...
   (check-equal? (sequence 3 11 2) (list 3 5 7 9 11) "Seuqence test")
   (check-equal? (sequence 3 8 3) (list 3 6) "Seuqence test")
   (check-equal? (sequence 3 2 1) '() "Seuqence test")
   ...
   (check-equal? (list-nth-mod (list 0 1 2 3 4) 11) 1 "list-nth-mod test")
   ;(check-equal? (list-nth-mod (list 0 1 2 3 4) -1) (error "list-nth-mod: negative number") "list-nth-mod test")
   ;(check-equal? (list-nth-mod '() 0) (error "list-nth-mod: empty list") "list-nth-mod test")
   ...
   (check-equal? (stream-for-n-steps ones -3) '() "stream-for-n-steps test")
   (check-equal? (stream-for-n-steps ones 0) '() "stream-for-n-steps test")
   (check-equal? (stream-for-n-steps count 5) (list 1 2 3 4 5) "stream-for-n-steps test")
   ...
   (check-equal? (stream-for-n-steps (stream-add-zero count) 3) (list (cons 0 1) (cons 0 2) (cons 0 3)) "stream-add-zero test")
   
   ; cycle-lists test
   (check-equal? (stream-for-n-steps (cycle-lists (list 1 2 3) (list "a" "b")) 3) (list (cons 1 "a") (cons 2 "b") (cons 3 "a")) 
                 "cycle-lists test")
   (check-equal? (stream-for-n-steps (cycle-lists (list 1 2) (list "a" "b")) 4) (list (cons 1 "a") (cons 2 "b") (cons 1 "a") (cons 2 "b")) 
                 "cycle-lists test")
   
   ; vector-assoc test
   (check-equal? (vector-assoc 4 (vector (cons 2 1) (cons 3 1) (cons 4 1) (cons 5 1))) (cons 4 1) "vector-assoc test")
   (check-equal? (vector-assoc 4 (vector (list 2 1) (cons 3 1) 4 (cons 5 1))) #f "vector-assoc test")
   (check-equal? (vector-assoc 1 (vector (cons 1 1) (cons 1 3) 1 (cons 5 1))) (cons 1 1) "vector-assoc test")
   ...
   (check-equal? (fassoc 1) (cons 1 "a") "cached-assoc test")
   (check-equal? (fassoc 1) (cons 1 "a") "cached-assoc test")
   (check-equal? (fassoc 1) (cons 1 "a") "cached-assoc test")
   (check-equal? (fassoc 2) (cons 2 "b") "cached-assoc test")
   (check-equal? (fassoc 3) (cons 3 "c") "cached-assoc test")
   (check-equal? (fassoc 4) (cons 4 "d") "cached-assoc test")
   (check-equal? (fassoc 3) (cons 3 "c") "cached-assoc test")
   (check-equal? (fassoc 4) (cons 4 "d") "cached-assoc test")
   (check-equal? (fassoc 1) (cons 1 "a") "cached-assoc test")
